...
{
    public class NagruzkaPrepods
    {
        BazaSoft bsManager;
        DataModels.Group CurrentGroup {  get; set; }
        int IdSpecialnost { get; set; }
        int Kurs { get; set; }
        System.Data.DataTable dt;

        string TableName = "prep_group_uchplan";
        string stringStatus;
        ...
        string sql_Procedure = "NAGRUSKA_PREPOD_FROM_GROUP";
        string sql_GetStatus = "SELECT COUNT(*) FROM {0} WHERE id_group={1}";
        string DeleteString = "DELETE FROM {0} WHERE id_group={1}";
        public NagruzkaPrepods(DataModels.Group group)
        {
            CurrentGroup = group;
            Init(); 
        }
        void Init()
        {
            ...
            string sql = string.Format(sql_GetStatus, TableName, CurrentGroup.Id);
            int nCount = bsManager.RunCommand(sql);
            if (nCount <=0)
                StringStatus="Нет данных";
            else
                StringStatus="Норма";
            dt =bsManager.getTableFromPprocedure(sql_Procedure, CurrentGroup.Id,"idgroup");
            AddColumns();
            InitFioColumn();
            dt.AcceptChanges();
        ...
        {
            string sql = string.Format(DeleteString, TableName, CurrentGroup.Id);
            using (MySqlConnection conn = new MySqlConnection(bsManager.MySqlConnectionString))
            {
                conn.Open();
    ...
    public class NagruzkaFullData
    {
        BazaSoft bsManager;
        System.Data.DataTable dt;
        string TableName = "prep_group_uchplan";
        string sql_Procedure = "NAGRUZKA_PREPODS_ALLGROUPS";
        public NagruzkaFullData()
        {
            Init();
        }
        void Init()
        {
            bsManager = new BazaSoft();
            dt =bsManager.getTableFromPprocedure(sql_Procedure);
            AddColumns();
            InitFioColumn();
            dt.AcceptChanges();
